...
// Returns the 4 passes' rgen shader paths (generate / temporal / spatial / output).
...
// Shader list shared across all ReSTIR passes (only rgen differs).
...
void ReSTIRGISphere::init() {
	...
	// The stub OBJ was loaded by RayTracer; discard it and substitute sphere
	// scene data so SceneDesc addresses point to correct geometry.
	...
	// Plane material indices come after all sphere materials.
	std::vector<PrimMeshInfo> prim_lookup(2);
	prim_lookup[0].index_offset  = 0;
	prim_lookup[0].vertex_offset = 0;
	prim_lookup[0].material_index = N;          // lower plane
	...
	prim_lookup[1].material_index = N + 1;      // upper plane
	...
	// ── 4. Scene dimensions (world_radius used by ReSTIR GI) ──────────────
...
void ReSTIRGISphere::create_accel(vk::BVH& tlas_out, std::vector<vk::BVH>& blases) {
	...
	// vertex_buffer holds plane_positions (glm::vec3, stride matches to_vk_geometry).
	...
	// ── TLAS: 3 instances ─────────────────────────────────────────────────
	// hitGroupIndex = instanceSBT + sbtOffset (sbtStride = 0 in traceRayEXT).
	// Sphere:       instanceSBT=0 → primary HG0, shadow HG1
	// Lower plane:  instanceSBT=2 → primary HG2, shadow HG3
	// Upper plane:  instanceSBT=2 → primary HG2, shadow HG3
	...
	// Instance 1: lower plane (translate to lower_plane_y)
	...
	instances[1].instanceCustomIndex     = 0;    // prim_lookup[0] = lower plane
	...
	// Instance 2: upper plane (Y-flip + translate to upper_plane_y)
	glm::mat4 upper_transform = glm::mat4(1.0f);
	upper_transform[1][1] = -1.0f;                          // flip Y axis
	upper_transform[3][1] = sphere_config.upper_plane_y;    // translate
	instances[2].transform               = vk::to_vk_matrix(upper_transform);
	instances[2].instanceCustomIndex     = 1;    // prim_lookup[1] = upper plane
...
void ReSTIRGISphere::render() {
	pc_ray.size_x                = Window::width();
	pc_ray.size_y                = Window::height();
	pc_ray.num_lights            = (int)lumen_scene->gpu_lights.size();
	pc_ray.random_num            = rand() % UINT_MAX;
	pc_ray.max_depth             = 6;   // sphere scene fixed depth
	...
	// ── Generate samples ──────────────────────────────────────────────────
	vk::render_graph()
		->add_rt("ReSTIRGISphere - Generate",
	...
	// ── Temporal reuse ────────────────────────────────────────────────────
	vk::render_graph()
		->add_rt("ReSTIRGISphere - Temporal",
	...
	// ── Output (compute pass – no RT shaders, no sphere_buffer needed) ────
	vk::render_graph()
		->add_compute("ReSTIRGISphere - Output",
		              {.shader = vk::Shader("src/shaders/integrators/restir/gi/output.comp"),
		               .dims   = {(uint32_t)std::ceil(Window::width() * Window::height() / 1024.0f), 1, 1}})
		.push_constants(&pc_ray)
...
void ReSTIRGISphere::destroy() {
	...
	// lumen_scene->vertex_buffer, index_buffer, compact_vertices_buffer,
	// materials_buffer, prim_lookup_buffer, mesh_lights_buffer are all
	// freed by LumenScene::destroy() (called after integrator->destroy()).
}
